...
compare_array=[]
for br in total_branch_list:
	branch_name = br.GetName()
	if branch_name.find("SuperCombinedMVA_BEST") != -1 or branch_name.find("All_COMB_BEST") != -1:
		compare_array.append(branch_name)


if not os.path.isdir(args.OutputDir+"SuperMVA/SuperCombinedMVA/"):os.makedirs(args.OutputDir+"SuperMVA/SuperCombinedMVA/")
DrawROCOverlaysFromROOT(args.InputFile,args.InputTree,args.OutputDir+"SuperMVA/SuperCombinedMVA/ROCOverlays_SuperCombinedMVAGains"+args.OutputExt,compare_array,signal_selection,bkg_selection)
